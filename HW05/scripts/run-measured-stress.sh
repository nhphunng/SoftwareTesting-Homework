...
threads="${STRESS_THREADS:-80}"
ramp_up="${STRESS_RAMP_UP_SECONDS:-240}"
duration="${STRESS_DURATION_SECONDS:-300}"
think_time="${STRESS_THINK_TIME_MS:-250}"
run_suffix="${RUN_SUFFIX:-}"
run_suffix="${run_suffix//[^A-Za-z0-9_-]/}"
artifact_name="23127194_Stress_20260812${run_suffix:+_$run_suffix}"
csv_file="$project_dir/data/scenario-c.local.csv"
raw_dir="$project_dir/results/raw/stress"
html_dir="$project_dir/results/html/stress/$artifact_name"
evidence_dir="$project_dir/evidence/stress"
...
for value in "$threads" "$ramp_up" "$duration"; do
  [[ "$value" =~ ^[1-9][0-9]*$ ]] || { echo "Stress thread/ramp/duration values must be positive integers." >&2; exit 1; }
done
[[ "$think_time" =~ ^[0-9]+$ ]] || { echo "Stress think-time must be a non-negative integer." >&2; exit 1; }
...
[[ -s "$csv_file" ]] || { echo "Local CSV is missing; provision it once with a local password." >&2; exit 1; }
if [[ -e "$raw_jtl" || -e "$html_dir" ]]; then
  echo "Stress output already exists; refusing to overwrite evidence." >&2
  exit 1
fi
...
if lsof -nP -iTCP:3000 -sTCP:LISTEN >/dev/null 2>&1; then
  echo "Port 3000 is already in use; stop the existing backend before Stress execution." >&2
  exit 1
fi
...
(
  cd "$sut_backend_dir"
  exec node server.js
) > /tmp/hw05-eshop-backend-stress.log 2>&1 &
backend_pid=$!

ready=false
for _ in {1..30}; do
  if grep -q 'Database initialized and seeded' /tmp/hw05-eshop-backend-stress.log 2>/dev/null \
...
[[ "$ready" == "true" ]] || { echo "Backend did not become ready." >&2; exit 1; }

(
  cd "$project_dir"
  USER_COUNT="$threads" JMETER_USER_PASSWORD="$local_password" scripts/provision-jmeter-users.sh
)
...
(
  cd "$project_dir"
  jmeter -n -t tests/23127194_Stress_20260812.jmx \
    -Jstress.threads="$threads" \
    -Jstress.ramp_up_seconds="$ramp_up" \
    -Jstress.duration_seconds="$duration" \
    -Jstress.think_time_ms="$think_time" \
...

{
  ...
  while true; do
    ...
    java_pid=$(pgrep -f 'ApacheJMeter.jar.*23127194_Stress_20260812.jmx' | head -n 1 || true)
    if [[ -n "$java_pid" ]]; then
      jmeter_stats=$(ps -p "$java_pid" -o %cpu= -o rss= 2>/dev/null | xargs || true)
    else
      jmeter_stats=""
    fi
    printf '%s,%s,%s\n' "$(date '+%Y-%m-%dT%H:%M:%S%z')" "${backend_stats// /,}" "${jmeter_stats// /,}"
...
if [[ "$jmeter_status" -ne 0 ]]; then
  echo "JMeter failed with status $jmeter_status; inspect $console_log." >&2
  exit "$jmeter_status"
fi

echo "Stress execution completed."
